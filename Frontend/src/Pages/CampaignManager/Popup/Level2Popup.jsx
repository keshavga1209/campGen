...
export default function Level2Prompt({ setCreatePopup, setLevel }) {
	...
	const [optimized_prompt_result, set_optimized_prompt_result] =
		useState(null);
	const [isLoading, setIsLoading] = useState(false);

	const getOptimizedPrompt = async () => {
		setIsLoading(true);
		const [err, res] = await request("POST", endpoints.OPTIMIZED_PROMPT, {
	...
	return (
		<div className="fixed top-0 left-0 flex justify-center items-center h-full w-full backdrop-blur-sm z-10">
			<div className="relative bg-white rounded-lg h-[40rem] w-[44rem] shadow-lg flex flex-col items-center">
				<button
				...
				<button
					className="bg-gray-200 hover:bg-gray-400 flex gap-2 items-center rounded-xl py-1 px-3 mt-8"
					onClick={(_) => {
						const idx =
							(currPrompt + 1) %
							optimized_prompt_result["items"].length;
						setCurrPrompt(idx);
						setText(
							optimized_prompt_result?.["items"]?.[idx]?.["text"]
						);
					}}>
					<TfiReload />
					Another prompt
				...
				<textarea
					className="bg-gray-200 rounded-lg p-2 mx-1 mt-8 grow-x"
					value={text}
				...
				<p className="bg-yellow-400/50 rounded-lg p-4 w-[38rem] mt-8 h-[8rem] overflow-y-auto">
					<b>Add-ons :</b>{" "}
					{optimized_prompt_result?.["missing_information"]}
				</p>
